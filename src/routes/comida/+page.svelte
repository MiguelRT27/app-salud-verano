<script lang="ts">
	...
	import { getDailyMacros, searchMealsByDate, initDB } from '$lib/db';
	...
	// Función para obtener meal por tipo
	function getMealByType(type: string) {
		return meals.find((meal) => meal.mealType === type);
	}

	// Array con pairing tipo + meal (puede ser undefined)
	$: mealsByType = mealTypes.map(type => ({ type, meal: getMealByType(type) }));
</script>

<section
	class="flex min-h-screen flex-col items-center space-y-8 bg-black pt-8 font-sans text-white"
>
	<h1 class="text-lg text-gray-400">Hoy, {dia}</h1>

	<!-- Card resumen macros diaria -->
	...
	{#each mealsByType as entry}
		<div class="summary-card w-full max-w-md px-6 py-4">
			...
			<button class="mt-2 text-sm text-blue-400 underline">[+] Añadir alimento</button>
		</div>
	{/each}
</section>
